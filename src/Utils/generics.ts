...
const IDGenerate = (userId?: string): [string, string] => {
	let e = "";
...
export const generateMessageIDV2 = (userId?: string, mode: "brainxiex" | string = "brainxiex"): string => {
	if (mode === "brainxiex") {
		return IDGenerate(userId).join("").substring(0, 20)
	}
	const data = Buffer.alloc(8 + 20 + 16)
	data.writeBigUInt64BE(BigInt(Math.floor(Date.now() / 1000)))
...
export const generateMessageID = (mode: "brainxiex" | string = "brainxiex") => {
	if (mode === "brainxiex") {
		return generateMessageIDV2(undefined, "brainxiex")
	}
